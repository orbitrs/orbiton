...
pub fn execute(args: DevArgs) -> Result<()> {
    ...
    println!("{} development server for project at {:?}", 
        style("Starting").bold().green(),
        project_dir);
    
    // Create a development server
    let server = DevServer::new(args.port, &project_dir)?;
    
    // Start the server in a separate thread
    let server_handle = server.start()?;
    
    println!("Development server running at {}", 
        style(format!("http://localhost:{}", args.port)).bold().blue().underlined());
...
fn setup_file_watching(project_dir: &Path) -> Result<()> {
    ...
    std::thread::spawn(move || {
        for event in rx {
            ...
            if should_rebuild {
                println!("{} project due to file changes", style("Rebuilding").bold().yellow());
