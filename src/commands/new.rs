...
fn generate_project(name: &str, template: &str, output_dir: &Path) -> Result<()> {
    ...
    let template_content = templates::get_template(template)
        .with_context(|| format!("Failed to get template: {}", template))?;
    ...
    for (file_path, content) in template_content {
        ...
        let rendered_content = render_template(&content, &vars)
            .with_context(|| format!("Failed to render template for file: {}", file_path))?;
