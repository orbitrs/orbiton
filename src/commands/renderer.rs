...
pub fn execute(args: RendererArgs) -> Result<()> {
    ...
    let renderer_type = match args.config.to_lowercase().as_str() {
        ...
        "auto" => "auto",
        _ => {
            return Err(anyhow::anyhow!("Invalid renderer configuration: {}. Valid options are: skia, wgpu, auto", args.config));
        }
    };
    ...
    if let Some(config_obj) = config.as_object_mut() {
        config_obj.insert("renderer".to_string(), serde_json::Value::String(renderer_type.to_string()));
    }
    ...
    println!("Renderer configured to {} in {:?}", 
