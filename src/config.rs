...
fn default_entry_point() -> String { "main.rs".to_string() }
fn default_dev_port() -> u16 { 3000 }
fn default_dev_host() -> String { "127.0.0.1".to_string() }
fn default_auto_open() -> bool { true }
fn default_hmr_enabled() -> bool { true }
fn default_hmr_debounce() -> u64 { 100 }
fn default_preserve_state() -> bool { true }
fn default_max_retries() -> u32 { 3 }
fn default_show_notifications() -> bool { true }
fn default_lint_enabled() -> bool { true }
...
impl OrbitonConfig {
    ...
    pub fn merge_with(&mut self, other: &OrbitonConfig) {
        ...
        if !other.hmr.ignore_patterns.is_empty() {
            self.hmr.ignore_patterns.extend(other.hmr.ignore_patterns.clone());
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if !src_path.exists() && self.project.src_dir != "src" {
            return Err(anyhow::anyhow!("Source directory does not exist: {}", self.project.src_dir));
        }
        
        // Validate HMR settings
        if self.hmr.debounce_ms > 5000 {
            return Err(anyhow::anyhow!("HMR debounce time too high (max 5000ms): {}", self.hmr.debounce_ms));
        }
